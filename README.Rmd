...
if (params$proxy_server == "noproxy") {
  config <- list()
} else {
  config <- httr::use_proxy(url = params$proxy_server, port = params$proxy_port)
}
```
# TargomoR

<!-- Badges -->
[![Build Status](https://api.travis-ci.org/cwthom/TargomoR.svg?branch=master)](https://travis-ci.org/cwthom/TargomoR)
...
```{r get-poygon-display, eval=FALSE}
...
```{r get-route-display, eval=FALSE}
...
```{r get-times-display, eval=FALSE}
